...
for macOS on Apple Silicon and on Intel, for Windows as an installer and as
a portable archive that needs none, and for Linux — five files, each carrying
the application icon, published with a `SHA256SUMS` taken over what the
release serves. The Linux tarball
